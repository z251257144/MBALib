...
/**
 *  使用方式：
    1、在你的应用程序中配置这个日志框架，通常在applicationDidFinishLaunching方法中配置
        1.1、发送日志语句到苹果的日志系统，以便它们显示在Console.app上
...
#define LOG_FLAG_ERROR    (1 << 0)  // 0...00001--只会看到DDlogError语句
#define LOG_FLAG_WARN     (1 << 1)  // 0...00010--只会看到DDLogError和DDLogWarn语句
#define LOG_FLAG_INFO     (1 << 2)  // 0...00100--会看到error、Warn和Info语句
#define LOG_FLAG_DEBUG    (1 << 3)  // 0...01000--会看到所有DDLog语句
#define LOG_FLAG_VERBOSE  (1 << 4)  // 0...10000--不会看到任何DDLog语句
